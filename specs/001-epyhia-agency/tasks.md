...
- [X] T094 [US3] Implement the `checkout_session` adapter in `epyhia/gate/adapters/stripe.py` — **not** approval-gated by design; `verify()` selects the order by `stripe_session_id` and asserts it exists and is paid (§4.4) — **deferred verification**: the adapter sets `defer_verification`, the row holds at `verifying` consuming no attempt, and `POST /webhooks/stripe` re-drives it (FR-041)
...
- [X] T116 [P] [US5] Test in `tests/integration/test_us5_cost.py`: every `agent_calls` row carries a non-null tier and cost, and **every planning-tier row is the Strategist's** (SC-007)
...
- [X] T118 [P] [US6] Add a second, **unrelated** brief fixture at `tests/fixtures/briefs/two.json` sharing no facts with `one.json` — a different kind of business entirely. It must be real enough to harvest tokens from; a placeholder name silently weakens the lint (R10)
...
- [X] T120 [P] [US6] Add the companion source scan in `tests/genericity/test_source_scan.py` — the same harvested tokens must not appear anywhere under `epyhia/`, `console/src/` or `video/src/`
- [X] T121 [US6] Write `eval/rubric.json` conforming to [contracts/eval-rubric.schema.json](./contracts/eval-rubric.schema.json) — every check carries `id`, `area`, `points`, `title`, `kind`, `assertion`, `evidence` and `required`; `evidence`-kind rows carry no score field. `area` and `points` come from [contracts/grading-rubric.md](./contracts/grading-rubric.md) §2 — all six area ids covered, per-area totals reconciling to it. Three of the six are entirely human-judged and are the easiest to omit (FR-067)
- [X] T136 [P] [US6] Test in `tests/eval/test_rubric_contract.py`: `eval/rubric.json` validates against the schema; every `area` is one of the six ids; **all six** appear at least once; and each area's `points` sum to that area's budget — with the expected ids and budgets **parsed out of [contracts/grading-rubric.md](./contracts/grading-rubric.md) §2**, never restated in the test, or the check is a copy grading a copy (FR-067)
...
- [X] T124 [US6] Implement the FR-062 second-brief assertions in `eval/eval.py` — the two runs differ in brand doc palette and alias and share no artifact `sha256`, with each run's deploy probe having read its expected name from **its own** brand doc row
...
- [X] T140 [P] [US6] Test in `tests/eval/test_report.py` against a stubbed record source: a failed required automated check still writes the report, names itself in the top summary, and exits non-zero; an `evidence` row resolving to nothing renders `missing` and leaves the exit status at zero; and no `evidence` row renders a score in either case (FR-063, FR-068)
...
- [X] T128 [P] Write `README.md` — the clone-to-running path from [quickstart.md](./quickstart.md) Prerequisites, and the explicit statement that the app starts with no credentials
...
- [X] T143 Make a prompt fix able to republish. `deploy_key(brief_hash, brand_doc.version, run.prompt_version)` reads `runs.prompt_version`, which ingest sets from the **Strategist's** active version alone ([briefs.py](../../epyhia/api/routers/briefs.py)) — so editing the Web Builder's instructions cannot change the deploy key, and a corrected page short-circuits onto the old publication rather than replacing it. Found on run `8d89a987` (2026-08-12): `prompts/web_builder/v2.jinja` had dropped v1's buy-button contract, the page shipped with `<a href>` links to a `POST`-only endpoint, and restoring the contract as `v3` republished nothing — the only lever available was a brand-doc version bump, which is a legitimate mechanism for a *brand* edit and the wrong one for "the instructions that built the page changed". `runs.prompt_version` is one column standing in for something genuinely per-agent, which `agent_calls.prompt_version` already records correctly per call (FR-055). Either key the deploy on the Web Builder's own prompt version or make the run's record a per-agent map — and note that **changing key derivation changes idempotency**, so it needs the §7.2 table updated in the same commit and the re-run assertions re-checked
- [X] T142 Give an operator a way to re-queue a **failed** stage, in the API and in the console's run view. Found on run `8d89a987` (2026-08-12): a bug in the Stripe product adapter landed the `money` task `failed`, and once fixed there was no way to restart it — the run sat complete in every other respect, deployed site and full pack included, with no route back to the one stage it was missing short of hand-writing a `tasks` row against production. `sweep_expired_leases` deliberately does not touch `failed` (it reclaims lapsed leases, and auto-retrying a handler exception is the loop the attempts cap exists to prevent), so this is a **deliberate** terminal state with a missing affordance rather than a bug in the sweeper. Re-queueing is safe by construction and that is the point: every gate key short-circuits on the same brief hash, so a re-queued stage resumes onto the rows that already succeeded and repeats no effect (§7.2, FR-044). This is the same shape as the defect it was found by — recovery that exists but nothing can reach
- [X] T145 Give an operator a way to re-run a **done** stage, in the API and in the console's run view. Found on run `9445c473` (2026-08-14), the first to hold a flagged artifact at stage completion: the Marketer wrote an unsupported claim the Reviewer held through both revisions, the copy stage completed `done` around its flagged artifact, and the site stage refused `UpstreamNotClean` forever. The spec's documented remedy — "correct the brief or the brand identity record and re-run, not hand-edit a flagged artifact in place" — had **no executable path**: T142's retry accepts only `failed`, resubmitting the brief dedups onto the same run and opens no tasks, and the US4 test "re-runs" by inserting a task row directly, which is why no test ever noticed. The operator's actual route was a raw `UPDATE tasks` in psql. Same shape as T142 — recovery that exists but nothing can reach — one state further along: T142 opened `failed`, this opens `done`, and the same §7.2 argument makes both safe (gate keys short-circuit, and agent calls memoise on the brand doc version, so a re-run without an edit replays while a re-run after one regenerates — which is exactly the remedy). Note the remedy also needs the newest-generation artifact lookup (`fix/newest-artifact-wins`): without it a re-run's clean artifact loses the `revision DESC` sort to the stale flagged generation and stays invisible to the stage downstream
- [X] T144 Give a run a terminal status. `runs.status` leaves `running` in exactly one direction: [budget.py](../../epyhia/cost/budget.py) sets `halted_budget` when spend crosses the ceiling, and **nothing anywhere sets `succeeded` or `failed`** — verified by grep, one writer in the whole codebase. So a run whose every stage is `done` reads as `running` forever, and so does one with a dead stage inside it. Found on run `c7751c83` (2026-08-12), where the console showed `running` while the `site` task sat `failed`; T142 gave the operator a way out of that stage, but the run containing it still never settles. Two things turn on this beyond the display: `_timeline`'s "a finished run has a finite timeline" branch ([runs.py](../../epyhia/api/routers/runs.py)) can only fire for a halted run, so every other SSE connection polls until the client goes away; and `statusVariant` in [runs.tsx](../../console/src/routes/runs.tsx) carries a `succeeded` case that can never render. The design question is what terminal means when a run's stages are a DAG rather than a line — "every stage `done`" is the obvious rule, but a `failed` stage the operator is about to re-queue is not a failed run, so the writer belongs where the queue settles a task (alongside `enforce_run_budget` in `run_once`) rather than in a periodic sweep that would race the retry button
- [X] T146 Give an operator a way to re-open **verification** on an action that failed at verify, in the API and in the console. Found during the outreach backfill (2026-08-16): Fly's proxy terminated TLS while uvicorn saw plain HTTP, so the sink's `request.url_for` handed back `http://` permalinks, `force_https` answered the publish adapter's verify `GET` with a 301, and `verify()` — correctly refusing to follow redirects, since an evidence URL must be canonical rather than eventually correct after a hop — burned all five attempts on every resumed `publish`. `execute()` had **succeeded every time**: the sink held the posts, the ids in the errors were theirs. The system did the work and then could not prove it to itself, and the config fix (`fix/sink-permalink-scheme`) left several actions sitting terminally `failed` with their effect already in the world. `failed` is terminal at the gate — `request()` short-circuits onto it and `resume()` returns it untouched — so **nothing reaches those rows**, and the only remedy was a raw `UPDATE actions SET state = 'verifying', verify_attempts = 0` against production. This is T142/T145's shape one layer down: tasks got their re-queue affordance, actions never got theirs. Scope note: the affordance must re-open *verification only* (state → `verifying`, attempts and error reset) and only for actions whose `execute()` evidence is already recorded — never re-execution, which idempotency keys already govern (§7.2) and which for `publish` would write duplicate posts for content the sink is already holding
- [X] T147 A re-queued task reports `done` over a `failed` action — a false green. Found auditing the deployed system post-`#50` (2026-08-16): four `publish` tasks read `done` while their action read `failed`. Cause: `request()` short-circuits on every terminal state and **returns** `_result(existing)`; every caller ignores the return value, so the handler completes and the worker writes `done` — the console shows a green stage over an unproved effect, the exact inversion of the gate's promise that nothing reads succeeded until proved in the world. Not T146 (that is "nothing can reach a failed action"; this is "a task lies about one"), and the defect is the gate's, so `deploy` and the Stripe trio have it latent even though only the 4 publish rows exhibit it. Fix in the gate, not in four call sites — "the gate refuses, not the agent" (FR-016), and four handlers each remembering to inspect a return value is the arrangement that produced this bug: `request()` returns the stored result only for `succeeded`; for `failed` and `denied` it raises `ActionTerminallyFailed(action_id, state)`, so the re-queued task lands `failed` naming the stuck action — legible, true, and the state T146's affordance acts on. `denied` raising matches deny's contract ("nothing executes, ever, for that key"). Sequencing consequence: re-queueing any failed outreach task before its action is healed manufactures more false greens, so nothing in the remediation runbook moves until this lands

- [X] T148 `POST /actions/{id}/reverify` must re-open a settled run, as `POST /tasks/{id}/retry` does. Found running T146's own remediation runbook against the deployed system (2026-08-16): all 19 `publish` actions re-verified `succeeded`, every stage healed to `done` over a proved action, false greens went to zero — and **all four runs stayed `failed`**, unreachable by anything. `settle_run` guards its write on `status = 'running'` so that settling never resurrects a `halted_budget` run ([settle.py](../../epyhia/queue/settle.py)), and its docstring already names the corollary — "the retry endpoint re-opens a settled run for the same reason" — which T146 shipped without honouring. The trap is that the omission is unrecoverable in place rather than merely untidy: the guard blocks the resume's settle, and once every stage is `done` there is no further task completion that would call `settle_run` again, so the run has no path out of `failed` at all. Same shape as T142/T145/T146 one more layer along — an affordance that re-opens work must re-open the run that work belongs to. `halted_budget` is deliberately not re-opened

- [X] T149 The Stripe webhook acknowledges signed events for runs this database does not contain — log and 200, never a 500 the provider retries for days. Found on the ghost-order incident (2026-08-16, run `3568c111`): a local stack holding production keys opened three checkout sessions, and their `checkout.session.completed` events arrived at the deployed endpoint naming runs only that stack had. `webhooks.py` already acknowledged events carrying *no* EPYHIA metadata, but metadata that is present and **foreign** fell through to `uuid.UUID(metadata["run_id"])` and then to the `INSERT INTO orders`, so each event 500ed on `orders_run_id_fkey` (Logfire, `evt_1U52UX…`) and Stripe retried it for days. Test mode is one account shared by every environment holding the key, so foreign events are not an incident but a permanent condition. Acknowledging is safe by construction: the session is created through the gate against a run already in this database, so an event for an unknown run cannot be a real order arriving early — there is no race in which ignoring it drops a sale. The `orders_run_id_fkey` constraint stays exactly as it is; it is the control that caught this, and the fix is to stop the webhook tripping over it, never to loosen it. Signature verification stays first, and the log line carries the event, session and foreign run ids so "foreign event ignored" reads differently from silence
