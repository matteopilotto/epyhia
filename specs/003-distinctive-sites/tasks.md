...
- [x] T041 Run the live evidence pass from quickstart §2–§4 (needs `ANTHROPIC_API_KEY` + the docker-compose stack): one fixture brief end-to-end — `site` and schema-valid `design_report` artifacts visible in the console, the two faces render offline with zero network requests, `site_critic` (and any `web_builder_revise`) rows in the cost view; both fixtures for the divergence check (SC-001: no shared palette, pairing, or archetype); the pre/post regression guard (SC-007: strictly fewer lint tells on the post-feature page); record outcomes in the feature's evidence notes
  ...
  **Superseded in part by the 4 × 2 sample of 2026-08-14 (PR #37).** "Both fixtures chose the same pairing" does not survive n=4 each: four distinct pairings in eight draws, modal sets disjoint, and the body face is not converged either. SC-001 remains NOT MET, now on the palette alone — modal palettes ΔE 8.2 apart on the accent and 1.7 on the ground. SC-001 itself has since been restated distributionally in `spec.md`, so this task's own parenthetical ("no shared palette, pairing, or archetype") is the pre-restatement wording.
...
- [x] T043 Test whether the prompt's illustrative triple is what the Strategist commits to (SC-001, follow-up to T041). The one traced `plan` stage drafted a dark high-contrast direction, a paper-and-ink one and a single saturated hue — matching `prompts/strategist/v3.jinja` lines 124–127 one for one — and committed to the middle example, which is the warm-cream family all five fixture-two samples have landed in. Sample the drafted directions and the committed palette across both fixtures (`plan` stage only, ~$0.20 each, and note that a driver calling `run_once()` directly must call `configure_tracing()` itself or it traces nothing), then vary the example triple in a `v4` prompt and re-sample. Two questions, both open on one observation: does the model narrate three directions every time, and does it stop landing on paper-and-ink when the prompt stops naming it? Record in `evidence.md`; a prompt change lands as a new version beside `v3` (FR-021)

  **Half done (2026-08-14, PR #37).** The committed palette is sampled: `scripts/sample_directions.py`, 4 × 2 at $2.912 ($0.36 a draw, not the $0.20 estimated here), results in `evidence.md`. It answers the second question partially — 6 of 8 draws land in the paper-and-ink family and 2 leave it coherently, so the pull is strong but not absolute. **Still open:** the drafted directions are not captured (the harness saves the brand doc, not the response text where the drafts live), and no `v4` exists. The re-sample is now one command, so varying the triple costs ~$2.90 and half an hour rather than a judgement call.

  **Closed 2026-08-14.** Both questions answered, and the experiment it asked for ran twice. *Does the model narrate three directions every time?* Yes — every one of the 20 draws sampled since capture landed in PR #39 (8 under `v4`, 12 here) wrote its drafting out in response text. *Does it stop landing on paper-and-ink when the prompt stops naming it?* No — `v4` removed the illustrative triple and made every measure worse or equal (`exp/strategist-v4-triple`, unmerged, not adopted), so the triple is refuted as the cause. The narrations then refuted the palette-instruction rewrite too: traceability is what the model already does unprompted. Widening the fixture set is what decided it — `three.json`, whose voice argues for the dark direction, draws 4/4 dark grounds at ΔE ≈ 90 from the cream family and passes every SC-001 clause against both existing fixtures (4 × 3 at $4.30). There is no palette default; the criterion was binding briefs that agree. Full record in `evidence.md`; the SC-001 rewording was signed off by the merge of PR #40 and **applied to `spec.md`**, rebinding the palette *and* archetype clauses to pairs of briefs that disagree in voice. **No prompt change is indicated by any of this.**
